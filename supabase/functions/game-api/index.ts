...
async function persistNextNews(gameId:string,nextNews:Record<string,unknown>|null){if(!nextNews)return;const turn=Number(nextNews.turn);if(!turn)return;const issue=(nextNews.issue??{}) as Record<string,unknown>;await db.from("news_board").upsert({game_id:gameId,turn,event_id:String(issue.id??""),headline:String(issue.headline??""),briefs:issue.briefs??[],issue,company_events:nextNews.companyEvents??[],surprise_event:nextNews.surpriseEvent??null,market_mood:nextNews.marketMood??{},sector_attention:nextNews.sectorAttention??{}},{onConflict:"game_id,turn"});const{data:existing}=await db.from("market_events").select("id").eq("game_id",gameId).eq("turn",turn).eq("event_type","automatic").maybeSingle();if(!existing)await db.from("market_events").insert({game_id:gameId,turn,event_type:"automatic",headline:String(issue.headline??""),payload:{eventId:issue.id,marketState:nextNews.marketState??{},sectorAttention:nextNews.sectorAttention??{}}});}
...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(action==="sync"){
      ...
      const{data:newsBoard}=await db.from("news_board").select("turn,issue,company_events,surprise_event,market_mood,sector_attention").eq("game_id",game.id).order("turn");const{count:surpriseUsed}=await db.from("market_events").select("id",{count:"exact",head:true}).eq("game_id",game.id).eq("event_type","surprise");
      const validNews=(newsBoard??[]).filter((row:{issue?:{id?:string}})=>Boolean(row.issue?.id));const result:{game:unknown;standings:unknown;finalResults:unknown;newsBoard:unknown;surpriseUsed:number;team?:unknown;teams?:unknown}={game,standings:await standings(game.id),finalResults:finalResults??[],newsBoard:validNews,surpriseUsed:surpriseUsed??0};
    ...
    if(action==="submit_portfolio"){
      ...
      const total=positions.reduce((sum:number,item:{weight?:number})=>sum+Number(item.weight??0),0);if(Math.abs(total-100)>.01)return reply({error:"포트폴리오 비중 합계는 100%여야 합니다."},400);
    ...
    if(action==="start"||action==="advance"){
      ...
      const surpriseEvent=body.surpriseEvent as Record<string,unknown>|null;if(surpriseEvent){if(game.current_turn===1)return reply({error:"첫 포트폴리오 편성에는 돌발 이슈를 사용할 수 없습니다."},409);const{count}=await db.from("market_events").select("id",{count:"exact",head:true}).eq("game_id",game.id).eq("event_type","surprise");if((count??0)>=2)return reply({error:"관리자 돌발 이슈는 게임당 최대 2회입니다."},409);const{data:usedThisTurn}=await db.from("market_events").select("id").eq("game_id",game.id).eq("turn",game.current_turn).eq("event_type","surprise").maybeSingle();if(usedThisTurn)return reply({error:"한 턴에는 돌발 이슈를 한 번만 사용할 수 있습니다."},409);await db.from("market_events").insert({game_id:game.id,turn:game.current_turn,event_type:"surprise",headline:String(surpriseEvent.headline??"돌발 이슈"),payload:surpriseEvent});await db.from("news_board").update({surprise_event:surpriseEvent}).eq("game_id",game.id).eq("turn",game.current_turn);}
      if(game.current_turn===1){for(const team of teams??[]){const portfolio=Array.isArray(team.portfolio)?team.portfolio:[];await db.from("teams").update({previous_portfolio:portfolio,submitted:false,turnover_rate:0}).eq("id",team.id);await db.from("team_standings").update({submitted:false,updated_at:new Date().toISOString()}).eq("team_id",team.id);}await persistNextNews(game.id,(body.nextNews??null) as Record<string,unknown>|null);await db.from("games").update({current_turn:2,base_rate:nextRate,updated_at:new Date().toISOString()}).eq("id",game.id);return reply({ok:true,newsReleased:true});}
      const ranked=[];
      ...
      const nextTurn=game.current_turn+1;const finished=nextTurn>game.total_turns;if(!finished)await persistNextNews(game.id,(body.nextNews??null) as Record<string,unknown>|null);await db.from("games").update({status:finished?"finished":"playing",current_turn:Math.min(nextTurn,game.total_turns),base_rate:nextRate,updated_at:new Date().toISOString()}).eq("id",game.id);
