...
export default function Home() {
  ...
  const { addTeam, configureGame } = useGame();
  const [panel, setPanel] = useState<"admin" | "team" | null>(null);
  const [teamName, setTeamName] = useState("");
  const [error, setError] = useState("");

  async function join(event: FormEvent) {
    event.preventDefault();
    if (!await addTeam(teamName)) { setError("팀명을 확인해주세요. 중복되었거나 참가 인원이 가득 찼습니다."); return; }
  ...
  return <main className="landing">
    ...
    <section className="hero-grid">
      ...
      <div className="entry-grid">
        ...
        <Link className="entry entry-display" href="/display"><span>03 / LIVE</span><MonitorUp/><strong>프로젝터 조회</strong><small>로그인 없이 바로 보기</small><ArrowRight className="entry-arrow"/></Link>
    ...
    {panel && <div className="modal-backdrop" onMouseDown={() => setPanel(null)}><section className="modal" role="dialog" aria-modal="true" onMouseDown={(e) => e.stopPropagation()}>
      ...
      <p className="eyebrow">{panel === "team" ? "PLAYER ENTRY" : "GAME MASTER"}</p>
      <h2>{panel === "team" ? "팀으로 참가하기" : "새 게임 만들기"}</h2>
      {panel === "team" ? <form onSubmit={join} className="form-stack">
        <label>팀명<input value={teamName} onChange={(e) => {setTeamName(e.target.value);setError("");}} placeholder="예: 여의도 불개미" autoFocus maxLength={16}/></label>
        {error && <p className="form-error">{error}</p>}
        <Button type="submit">대기실 참가 <ArrowRight size={16}/></Button>
        <p className="form-note">별도 계정이나 비밀번호 없이 팀명만 입력합니다.</p>
      </form> : <form className="form-stack" onSubmit={async(e) => {e.preventDefault();const form=new FormData(e.currentTarget);await configureGame(Number(form.get("initialBudget")),Number(form.get("minimumTurnover")),Number(form.get("maxTeams")));router.push("/admin/lobby");}}>
        <label>게임명<input defaultValue="제1회 투자왕 결정전"/></label>
        <div className="form-row"><label>총 턴 수<select defaultValue="8"><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option></select></label><label>최대 팀 수<select name="maxTeams" defaultValue="12">{Array.from({length:11},(_,index)=>index+2).map(value=><option key={value}>{value}</option>)}</select></label></div>
        ...
        <Button type="submit" variant="gold">게임 만들기 <ArrowRight size={16}/></Button>
      </form>}
    </section></div>}
  </main>;
}
