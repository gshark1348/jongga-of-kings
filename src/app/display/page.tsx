...
export default function Display() {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(
      () => setMode((value) => (value + 1) % modes.length),
      10000,
    );
    ...
    return (
      <main className="display-page">
        <div className="display-header">
          <Brand />
          <span className="mono">PUBLIC DISPLAY · {gameCode || "NO GAME"}</span>
        </div>
        <section className="display-waiting">
          <div>
            ...
            <div className="display-game-code">
              <small>GAME CODE</small>
              <strong>{gameCode}</strong>
            </div>
            <p>
              현재 {teams.length}개 팀이 개장을 준비하고 있습니다.
            </p>
          </div>
        </section>
        <div className="ticker">
          <div className="ticker-track">
            <span>종가의 제왕</span>
            <span>게임 코드와 팀명을 입력하면 즉시 참가 목록에 추가됩니다</span>
            <span>종가의 제왕</span>
            <span>게임 코드와 팀명을 입력하면 즉시 참가 목록에 추가됩니다</span>
          </div>
        </div>
    ...
    <main className="display-page">
      <div className="display-header">
        ...
        <div className="display-meta">
          ...
          <span>{initialSetup ? "INITIAL SETUP" : `제${turn}턴 조간`}</span>
        </div>
      </div>
      {status === "calculating" ? (
        <section className="display-waiting">
          <div>
            <p className="eyebrow">MARKET CALCULATION</p>
            <h1>
              시장이 움직이고
              <br />
              있습니다
            </h1>
            <p>모든 종목에 새로운 이슈의 여파가 반영됩니다.</p>
          </div>
        </section>
      ) : initialSetup ? (
        <section className="display-waiting preopen-display">
          <div>
            <p className="eyebrow">INITIAL PORTFOLIO</p>
            <h1>
              뉴스 공개 전,
              <br />첫 전략을 세우는 시간
            </h1>
            <p>
              각 팀이 초기 예산을 배분하고 있습니다. 모든 팀이 제출하면 첫 시장
              뉴스가 공개됩니다.
            </p>
          </div>
            ...
            {teams.map((team) => (
              <div className="rank-row" key={team.id}>
                ...
                <div>
                  ...
                  <small>
                    {team.submitted ? "포트폴리오 확정" : "편성 중"}
                  </small>
                </div>
                <span className={team.submitted ? "up" : ""}>
                  {team.submitted ? "완료" : "대기"}
                </span>
              </div>
            ))}
          </aside>
        ...
        issue && (
          <>
            <section className="editorial-grid">
              <div className="newspaper">
                ...
                <div
                  ...
                  style={{
                    backgroundImage: `linear-gradient(180deg,transparent,rgba(20,19,17,.48)),url(${issue.imageUrl})`,
                  }}
                />
              ...
              <aside className="ranking">
                ...
                {teams.map((team) => (
                  <div className="rank-row" key={team.id}>
                    ...
                    <div>
                      <strong>{team.name}</strong>
                      <small>₩{(team.assets / 1000000).toFixed(2)}M</small>
                    </div>
                    <Delta value={team.totalReturn} />
                  </div>
                ))}
              </aside>
            </section>
            <div className="market-strip">
              ...
              <div>
                <small>최고 산업</small>
                <strong className="up">AI +5.90%</strong>
              </div>
              <div>
                <small>시장 심리</small>
            ...
            <div className="ticker">
              <div className="ticker-track">
                {[...companies.slice(0, 10), ...companies.slice(0, 10)].map(
                  (company, index) => (
                    <span key={`${company.id}-${index}`}>
                      {company.name} ₩{company.price.toLocaleString()}{" "}
