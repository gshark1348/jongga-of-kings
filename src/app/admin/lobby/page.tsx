...
export default function AdminLobby() {
  const { teams, startGame, initialBudget, maxTeams, connectionMode } = useGame(); const router = useRouter(); const [copied,setCopied]=useState(false); const slots=Array.from({length:Math.max(0,maxTeams-teams.length)});
  return <><SiteHeader admin/><Shell><PageHeader eyebrow="GAME MASTER · LOBBY" title="개장 준비실" description="팀들이 모이면 관리자가 시장의 문을 엽니다." actions={<Link href="/display"><Button variant="paper"><MonitorUp size={16}/> 프로젝터 열기</Button></Link>}/>
  <div className="metric-grid"><div className="metric"><span>게임 코드 {copied&&"· 복사됨"}</span><strong>JONGGA</strong> <button aria-label="코드 복사" onClick={()=>{navigator.clipboard?.writeText("JONGGA");setCopied(true)}}><Copy size={14}/></button></div><div className="metric"><span>초기 지급 예산</span><strong>₩{(initialBudget/100000000).toFixed(initialBudget<100000000?1:0)}억</strong></div><div className="metric"><span>연결 상태</span><strong>{connectionMode==="supabase"?"SUPABASE":"LOCAL"}</strong></div><div className="metric"><span>참가 현황</span><strong>{teams.length} / {maxTeams}</strong></div></div>
